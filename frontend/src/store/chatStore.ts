...
function createChatStore(contextType: string) {
  return create<ChatState>((set, get) => ({
    ...
    loadThreads: async () => {
      ...
      try {
        const { threads } = await conversationsApi.getThreads(contextType);
    ...
    createThread: async () => {
      try {
        const { thread } = await conversationsApi.createThread(undefined, contextType);
        set((state) => ({
...
// Default chat store (backward compatible)
export const useChatStore = createChatStore('chat');

// Meal prep store
export const useMealPrepStore = createChatStore('meal-prep');
